...
import {
  FSRS, FSRS_W, mergeCards, mergePracticeDays, totalForDay, studiedOnDay, localDateStr, today,
} from "./srs.js";

const card = (over = {}) => ({
  id: "c1", word: "w", translation: "t", phrase: "p",
  stability: 0, difficulty: 0, reps: 0, dueDate: today(),
  lastReview: null, firstReviewedAt: null, learningStep: 0,
  priority: false, suspended: false, modifiedAt: 1000, ...over,
});
...
describe("FSRS.review — graduated cards", () => {
  const graduated = card({ reps: 3, stability: 10, difficulty: 5, lastReview: "2026-05-20", firstReviewedAt: "2026-01-01", learningStep: 0 });
...
describe("mergePracticeDays", () => {
  ...
  it("takes per-device max across devices", () => {
    const m = mergePracticeDays({ "2026-01-01": { devA: 5, devB: 2 } }, { "2026-01-01": { devB: 9 } });
